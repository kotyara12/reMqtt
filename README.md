...
```
С помощью этого клиент MQTT будет автоматически запущен или остановлен по событиям подключения к WiFi сети. Кроме того, задача MQTT клиента сама отправляет события подключения или отключения к MQTT брокеру для оповещения других подсистем прошивки.
...
**Публикация сообщения в топик** ```topic``` с заданными параметрами ```QoS``` и ```retained```. Если установлен бит ```forced```, то попытка отправкии сообщения будет предпринята немедленно, _и в контексте вызывающей задачи_, иначе сообщение будет поставлено в очередь и отпавлено позже _в контексте задачи MQTT клиента_. При ```free_topic = true``` переданная строка ```char *topic``` будет удалена из кучи после отправки. Аналогично, при ```free_payload = true``` переданная строка ```char *payload``` будет удалена из кучи после отправки.
```
